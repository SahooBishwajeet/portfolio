...
const COMMANDS = ["roasted", "help", "cat about.me", "ls -la projects", "whoami", "curl repo", "neofetch", "clear"];
    ...
    if (input.startsWith("rm -rf") && input.trim() !== "rm -rf") {
        if (isSudo) {
            if (input === "rm -rf project" && !bareMode) {
                ...
                writeLines(["OMG!!! The project file is deleted.....Jokes on You, You can't do shite", "<br>"])
            } else {
                if (bareMode) {
...
const initEventListeners = () => {
    ...
    if (PRE_USER) {
        PRE_USER.innerText = main.username;
    }
    ...
    console.log(`%cPassword: ${main.password}`, "color: red; font-size: 20px;");
}
